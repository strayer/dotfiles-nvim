...
- The Neo-tree migration is implemented in isolated commit `1bf68ef`
  ...
  (`refactor(markdown): remove browser preview`), and native filetype cleanup is
  implemented in `2142857`
...
- The MiniFiles post-cleanup trial is active in `059e1a1`
  ...
  UI2 trial is active in `8576c82` (`feat(ui): trial native UI2`), with its
  idle command line hidden by `1c681c4`; its real-use decision is pending.
...
| Native UI2 post-cleanup trial | Active | `8576c82`, extended by `1c681c4`; `cmdheight=0` hides the idle command line, without message-routing customization. |
...
cleanup. Browser-preview removal reduced it to 40, and filetype cleanup reduced
it to 34. Development-tool cleanup reached the target 26 entries. Before
verifying the AI commit, `:Lazy restore` reconciled the shared installed state to
...
| `nvim-lint` | Diagnostics where LSPs are missing/subpar | Markdown/Dockerfile loading and scheduled initial lint completed in `d03ed1b`. |
...
- 2026-07-10, current `main` branch: official
  [WhichKey README](https://raw.githubusercontent.com/folke/which-key.nvim/main/README.md).
...
- 2026-07-10, installed Neovim 0.12.4: official
  ...
  experimental/private API status. The `cmdheight` documentation also marks
  zero as experimental and says it works better with UI2 enabled.
...
in `101d600`. Filetype entries were removed in `2142857`; development-tool
entries were removed in `d03ed1b`. The target graph reduction is complete.
...
6. `refactor(markdown): remove browser preview` - complete in `101d600`.
   - Remove `markdown-preview.nvim`; Markview remains the core in-editor Markdown
     renderer.
7. `refactor(filetypes): prefer native language support` - complete in
   `2142857`.
   ...
   - Add native Caddyfile detection and retain Tree-sitter Manager's Caddy
     parser path.
8. `refactor(dev-tools): remove unused tools and narrow linting` - complete in
   `d03ed1b`.
...
- Active: trial Neovim 0.12 UI2 from `8576c82`, with `cmdheight=0` added in
  `1c681c4`. Noice is gone, but UI2 and zero command height remain experimental;
  verify native messages, `g<`, `:messages`, shell output, prompts, Tmux, and
  Neovide before deciding. Do not copy unrelated `winborder`, progress-status,
  or completion-popup changes: existing retained systems already own those
  responsibilities.
...
- Keymaps have one canonical definition as normal Neovim or plugin mappings.
  The discovery layer observes descriptions rather than owning a parallel
...
| P33 | `noice.nvim` | Command/message/LSP UI | Remove | Buggy in practice; native 0.12 UI2 trial active in `8576c82`, with `cmdheight=0` added in `1c681c4` |
...
- Post-cleanup update: `613ced9` completed that architecture rewrite. Normal
...
| I09 | Neovim 0.12 UI2 | Experiment only after plugin removals; decide whether the private/experimental UI is stable enough to enable. | Active trial from `8576c82`, with `cmdheight=0` added in `1c681c4`; real-use decision pending |
